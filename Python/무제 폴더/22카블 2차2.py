...
REPLY_WEIGHT = 0.3 # 대답을 유보하는 기간 가중치 ... 최대한 쌓아두었다가 결정하기 위함
AMOUNT_SUM = 91 # 미만이면 처리 안 함
NORMAL = 0.3
...
for day in range(1, ED + 1):
    ...
    for req in reqs[day]:
        # 기준치 미달인데 호텔이 원할하지 않을 경우
        if get_amount(req) < AMOUNT_SUM and hotel_filled[day] < W*H*NORMAL:
            # 거절
            replies.append({"id": req["id"], "reply": "refused"})
            continue
...
print(score(auth_key), REPLY_WEIGHT, AMOUNT_SUM, NORMAL)